...
if BALANCE_WITH_OVERSAMPLING:
    X_tr = X_train_dir.copy()
    y_tr = y_train_dir.copy()

    # целевой размер = макс. из классов
    ...
    for cls in [0, 1, 2]:
        mask_c = (y_tr == cls)
        X_c, y_c = X_tr[mask_c], y_tr[mask_c]
        if len(y_c) == 0:
            continue
        if len(y_c) < target_n:
            X_res, y_res = resample(X_c, y_c, replace=True, n_samples=target_n, random_state=42)
        else:
            X_res, y_res = X_c, y_c
    ...
    logging.info(f"Oversampling: class counts before={counts.tolist()}, after={np.bincount(y_train_bal, minlength=3).tolist()}")
else:
...
# Температура (0.5–1.0). Ниже — сильнее усиление минорных классов.
TEMP = 0.45
train_counts = np.bincount(y_train_dir, minlength=3)
train_priors = train_counts / train_counts.sum()
alpha = (train_priors.mean() / np.clip(train_priors, 1e-6, 1.0)) ** TEMP

adj_probs = ens_probs * alpha  # масштабируем классовые вероятности
adj_probs = adj_probs / adj_probs.sum(axis=1, keepdims=True)

# === Margin decision rule поверх adj_probs ===
p0, p1, p2 = adj_probs[:, 0], adj_probs[:, 1], adj_probs[:, 2]
MARGIN = 0.08   # 8 п.п. зазор против класса 1
...
ens_pred = adj_probs.argmax(axis=1)  # используем скорректированные вероятности
...
for thr_map in cls_thr_sets:
    sel = (
        ((ens_pred==0) & (ens_probs[:,0] >= thr_map[0])) |
        ((ens_pred==1) & (ens_probs[:,1] >= thr_map[1])) |
        ((ens_pred==2) & (ens_probs[:,2] >= thr_map[2]))
    )
    if sel.sum():
...
test_news['correct'] = (test_news['pred'] == test_news['direction_cls'])
