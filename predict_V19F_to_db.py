...
def _load_news(engine: sqlalchemy.Engine) -> pd.DataFrame:
    ...
    def _get_first_existing_column(engine: sqlalchemy.Engine, table: str, candidates: Sequence[str]) -> Optional[str]:
        inspector = inspect(engine)
        cols = {c["name"] for c in inspector.get_columns(table)}
        for c in candidates:
            if c in cols:
    ...
    event_col      = _get_first_existing_column(engine, table, ["headline", "event", "event_title", "title", "name"])
    event_key_col  = _get_first_existing_column(engine, table, ["event_key", "eventId", "event_id", "event_code", "eventCode", "key"])

    # важность: сначала явные поля, иначе impact_score
    imp_total_col  = _get_first_existing_column(engine, table, ["imp_total", "importance_total", "importance", "impact_total"])
    impact_score_col = _get_first_existing_column(engine, table, ["impact_score"])

    imp_calc_col   = _get_first_existing_column(engine, table, ["imp_calculated", "calc_importance", "importance_calc"])
    imp_trend_col  = _get_first_existing_column(engine, table, ["imp_trend", "importance_trend"])
    direction_col  = _get_first_existing_column(engine, table, ["direction"])
    magnitude_col  = _get_first_existing_column(engine, table, ["magnitude", "magnitude_pips", "price_move_pips"])
    actual_col     = _get_first_existing_column(engine, table, ["actual", "actual_value", "value"])
    dependence_col = _get_first_existing_column(engine, table, ["dependence", "dependency", "dependent"])
    ...
    select_parts = [
        ...
        # imp_total: если «родного» нет — берём impact_score
        (f"{imp_total_col} AS imp_total" if imp_total_col
         else (f"{impact_score_col} AS imp_total" if impact_score_col else "0.0 AS imp_total")),
    ...
    # окно: прошлые N минут + будущие H часов
    query = text(
        f"""
        SELECT {base_select}
        FROM {table}
        WHERE timestamp_utc BETWEEN UTC_TIMESTAMP() - INTERVAL :past_m MINUTE
                                AND UTC_TIMESTAMP() + INTERVAL :future_h HOUR
        ORDER BY timestamp_utc
        """
    )
    params = {"past_m": FUTURE_PAST_MINUTES, "future_h": FUTURE_HOURS}

    df = pd.read_sql(query, engine, params=params)
    if df.empty:
        logging.warning("No news fetched for current mode")
        return df
    # Время в датафрейме → naive UTC + округление
    df["timestamp_utc"] = (
        pd.to_datetime(df["timestamp_utc"], utc=True)
          .dt.tz_convert(None)          # убрать таймзону
          .dt.round("min")
    )
    ...
    # числовые поля
    for col in ["imp_total", "imp_calculated", "imp_trend", "direction", "magnitude", "actual"]:
        if col not in df:
            df[col] = 0.0
        df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0.0)

    # нормализация важности (если >1 — min-max в [0..1])
    if after_time_cnt:
        mx = float(df["imp_total"].abs().max())
        if mx > 1.0:
            mn = float(df["imp_total"].min())
            span = (mx - mn) if mx > mn else 1.0
            df["imp_total"] = ((df["imp_total"] - mn) / span).clip(0.0, 1.0)

    # НЕ фильтруем по imp_total здесь (IMP_TOTAL_THRESHOLD=0.0)
    ...
    logging.info(
        ...
        raw_cnt, after_time_cnt, after_imp_cnt, start_utc, end_utc
    )
    logging.info(
        "news_tradingeconomics mapping: event=%s, event_key=%s, imp_total_src=%s, rows=%d",
        event_col, event_key_col, (imp_total_col or impact_score_col or "const0"), len(df)
    )
    logging.info(
        "news window: raw=%d, after_time=%d, time=[%s..%s], df.min=%s, df.max=%s",
        raw_cnt, after_time_cnt, start_utc, end_utc,
        df["timestamp_utc"].min() if after_time_cnt else None,
        df["timestamp_utc"].max() if after_time_cnt else None,
    )
    return df
...
def main() -> None:
    ...
    logging.info(
        "preds candidates: n=%d; dir_prob[min=%.3f max=%.3f mean=%.3f]; mag[min=%.3f max=%.3f mean=%.3f]",
        0 if preds is None else len(preds),
        0.0 if preds is None or preds.empty else float(preds["direction_prob"].min()),
        0.0 if preds is None or preds.empty else float(preds["direction_prob"].max()),
        0.0 if preds is None or preds.empty else float(preds["direction_prob"].mean()),
        0.0 if preds is None or preds.empty else float(preds["magnitude_pred"].min()),
        0.0 if preds is None or preds.empty else float(preds["magnitude_pred"].max()),
        0.0 if preds is None or preds.empty else float(preds["magnitude_pred"].mean()),
    )
