...
).drop(columns=['timestamp_utc'], errors='ignore').rename(columns={'close':'close_pre5'})
...
news = pd.merge_asof(
    ...
    direction='backward', tolerance=pd.Timedelta('90min')
).drop(columns=['timestamp_utc'], errors='ignore').rename(columns={'close':'close_pre15'})

# --- убрать дубликаты колонок и гарантировать 1D-колонки для арифметики ---
news = news.loc[:, ~news.columns.duplicated(keep='first')].reset_index(drop=True)

for _c in ['close_pre5', 'close_pre15', 'close_t0']:
    if _c in news.columns and isinstance(news[_c], pd.DataFrame):
        news[_c] = news[_c].iloc[:, 0]

for _c in ['close_pre5', 'close_pre15', 'close_t0']:
    news[_c] = pd.to_numeric(news[_c], errors='coerce')
...
# Сортируем по типу события (event_key) и времени
...
# Порог динамический
THR_MIN_PIPS = 1.2
THR_COEFF    = 0.40   # сильнее завязка на волу
THR_MAX_PIPS = 6.0    # верхний клип, чтобы не уходить в космос
...
thr_dyn = np.clip((base_thr * scale).astype(float), THR_MIN_PIPS, THR_MAX_PIPS)
...
for c in ['ret_pre_5m','ret_pre_15m','ret_pre_15m_norm']:
    ql, qh = news[c].quantile([0.01, 0.99])
    news[c] = news[c].clip(ql, qh)
...
# --- выбери, чем учимся: фикс 15м или динамика до следующей новости ---
USE_DYNAMIC_LABELS = True  # ← если False, останется как было (15 минут)
...
if const_cols:
    ...
    X = news[FEATURES].copy()  # пересобираем без констант
...
# Базовые веса + boost одиночек + УСИЛЕННЫЙ boost для крайних классов
w_train = np.array([cw_map[c] for c in y_dir[mask_time]])
# мягче буст одиночек и сильнее края
w_train = w_train * np.clip(news.loc[mask_time, 'singleton_boost'].to_numpy(), 1.0, 1.3)
class_boost = {0: 1.8, 1: 1.0, 2: 1.6}
w_train *= np.vectorize(class_boost.get)(y_train_dir)
# нормализуем, чтобы веса не «задушили» модели
w_train = w_train * (len(w_train) / w_train.sum())

# дополнительный коэффициент за плотность (±15м): 1 → буст, 2-3 → слегка, >3 → штраф
density = news.loc[mask_time, 'news_count_pm15m'].to_numpy()
w_density = np.where(density <= 1, 1.50, np.where(density <= 3, 1.10, 0.85))
w_train *= w_density

# === BALANCING: класс-равный oversampling на трейне ===
from sklearn.utils import resample

BALANCE_WITH_OVERSAMPLING = True  # включаем

if BALANCE_WITH_OVERSAMPLING:
    # упакуем X, y и веса в один датафрейм, чтобы ресемплить синхронно
    train_df = X_train_dir.copy()
    train_df['__y__'] = y_train_dir
    train_df['__w__'] = w_train  # веса уже выведены под mask_time

    counts = train_df['__y__'].value_counts()
    target_n = counts.max()

    parts = []
    for cls, grp in train_df.groupby('__y__'):
        if len(grp) < target_n:
            grp_res = resample(grp, replace=True, n_samples=target_n, random_state=42)
        else:
            grp_res = grp
        parts.append(grp_res)

    train_bal = pd.concat(parts, axis=0).sample(frac=1.0, random_state=42).reset_index(drop=True)
    X_train_bal = train_bal.drop(columns=['__y__', '__w__'])
    y_train_bal = train_bal['__y__'].to_numpy()
    w_train_bal = train_bal['__w__'].to_numpy()

    logging.info(f"Oversampling: class counts before={np.bincount(y_train_dir, minlength=3).tolist()}, "
                 f"after={np.bincount(y_train_bal, minlength=3).tolist()}")
else:
    X_train_bal = X_train_dir
    y_train_bal = y_train_dir
    w_train_bal = w_train
...
try:
    brf.fit(X_train_bal, y_train_bal, sample_weight=w_train_bal)
except TypeError:
    # на случай старой версии imblearn без поддержки sample_weight
...
ens_probs = (rf_probs + xgb_probs + brf_probs) / 3.0

# === Prior correction (anti-collapse): усиливаем редкие классы ===
train_counts = np.bincount(y_train_dir, minlength=3)  # priors с оригинального train — норм
train_priors = train_counts / train_counts.sum()

TEMP = 0.45  # 0.45–0.6
alpha = (train_priors.mean() / np.clip(train_priors, 1e-6, 1.0)) ** TEMP

adj_probs = ens_probs * alpha
adj_probs = adj_probs / adj_probs.sum(axis=1, keepdims=True)

# лог — сырые и скорректированные распределения
logging.info(f"Train priors: {np.round(train_priors,4).tolist()}, alpha: {np.round(alpha,3).tolist()}")
logging.info(f"Pred counts (raw): {np.bincount(ens_probs.argmax(axis=1), minlength=3).tolist()}")
logging.info(f"Avg probs (raw p0,p1,p2): {[round(float(x),4) for x in ens_probs.mean(axis=0)]}")
logging.info(f"Pred counts (adj): {np.bincount(adj_probs.argmax(axis=1), minlength=3).tolist()}")
logging.info(f"Avg probs (adj p0,p1,p2): {[round(float(x),4) for x in adj_probs.mean(axis=0)]}")

# === Margin decision rule поверх adj_probs (чтобы 1 не «съедал» всё)
p0, p1, p2 = adj_probs[:, 0], adj_probs[:, 1], adj_probs[:, 2]
MARGIN = 0.08
ens_pred = np.where(
    (p2 - p1) >= MARGIN, 2,
    np.where((p0 - p1) >= MARGIN, 0, 1)
)
...
logging.info(f"Train priors: {np.round(train_priors,4).tolist()}, alpha: {np.round(alpha,3).tolist()}")
pred_counts = np.bincount(ens_pred, minlength=3)
avg_probs = adj_probs.mean(axis=0)
logging.info(f"Pred counts (0,1,2) adj: {pred_counts.tolist()}")
logging.info(f"Avg probs adj (p0,p1,p2): {[round(float(x),4) for x in avg_probs]}")




pred_counts = np.bincount(ens_pred, minlength=3)
avg_probs = ens_probs.mean(axis=0)
logging.info(f"Pred counts (0,1,2): {pred_counts.tolist()}")
logging.info(f"Avg probs (p0,p1,p2): {[round(x,4) for x in avg_probs]}")
...
for thr in [0.50, 0.55, 0.60, 0.65, 0.70, 0.75]:
    ...
    f1_t  = f1_score(y_test_dir[mask], ens_pred[mask], average='macro')
    cov = mask.mean()
    logging.info(f"Conf>={thr:.2f}: coverage={cov:6.2%} n={mask.sum():3d} acc={acc_t:.3f} f1={f1_t:.3f}")

# === Class-specific thresholds (диагностика) ===
cls_thr_sets = [
    {0:0.55, 1:0.62, 2:0.55},
    {0:0.60, 1:0.64, 2:0.58},
]
for thr_map in cls_thr_sets:
    sel = (
        ((ens_pred==0) & (adj_probs[:,0] >= thr_map[0])) |
        ((ens_pred==1) & (adj_probs[:,1] >= thr_map[1])) |
        ((ens_pred==2) & (adj_probs[:,2] >= thr_map[2]))
    )
    if sel.sum():
        acc_t = accuracy_score(y_test_dir[sel], ens_pred[sel])
        f1_t  = f1_score(y_test_dir[sel], ens_pred[sel], average='macro')
        logging.info(f"Class-thresholds {thr_map}: coverage={sel.mean():.2%} n={sel.sum()} acc={acc_t:.3f} f1={f1_t:.3f}")
