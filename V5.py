...
# Важно: делаем merge ТОЛЬКО если колонок еще нет
...
# Проверка: убеждаемся что колонки одномерные
for col in ['close_t0', 'close_pre5', 'close_pre15']:
    if col in news.columns:
        # Если вдруг 2D, берём первый столбец
        if isinstance(news[col].iloc[0], (list, np.ndarray)) or news[col].apply(lambda x: hasattr(x, '__len__') and not isinstance(x, str)).any():
            logging.warning(f"Column {col} is multidimensional, taking first column")
            news[col] = news[col].apply(lambda x: x[0] if isinstance(x, (list, np.ndarray)) else x)
...
# Импульсы до новости - ИСПРАВЛЕНО
