...
def _load_news(engine: sqlalchemy.Engine) -> pd.DataFrame:
    ...
    if FUTURE_MODE:
        query = text(
            ...
            SELECT {base_select}
            FROM {table}
            WHERE timestamp_utc BETWEEN UTC_TIMESTAMP() - INTERVAL :past_minutes MINUTE
                                      AND UTC_TIMESTAMP() + INTERVAL :future_hours HOUR
            ORDER BY timestamp_utc
        """
        )
        params = {"future_hours": FUTURE_HOURS, "past_minutes": FUTURE_PAST_MINUTES}
